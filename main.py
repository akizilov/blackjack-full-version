...
def hit(card_pile, dealers_pile):
    ...
    if len(card_pile) == 2 and card_pile[0] == card_pile[1]:
        if cash - bet >= 0 :
            selection = int(input("Would you like to split your cards? "))
        print()
    if total_card_pile == 21:
        ...
        if total_dealers_pile <= 21 and total_dealers_pile > total_card_pile or is_blackjack_dealer == True and is_blackjack_user == False :
            print("Dealer won!")
        elif total_card_pile <= 21 and total_dealers_pile < total_card_pile or is_blackjack_user == True and is_blackjack_dealer == False:
            if is_blackjack_user == True:
                print("BLACKJACK!")
                cash += bet*1.5
            else:
                print("User won!")
        
        if total_card_pile == total_dealers_pile or is_blackjack_user == True and is_blackjack_dealer == True:
            print("It's a tie")
        status = "done"
...
def stand(card_pile, dealers_pile, status): #NOT SENDING BACK THE STATUS CURRENTLY
    global cash
    ...
    while dealers_total < 16: #dealer keeps hitting until the total is at least 16
        card_select = random.randint(1,11) #add a check for aces
...
print()
print("Dealer's cards: "+str(dealers_cards))
print("Dealer's total: "+str(card_addition(dealers_cards)))
print()
print("User's cards: "+str(current_pile))
print("User's total: "+str(total_card_pile))
print()    

while len(current_pile) > 0 and current_pile[0] == current_pile[1]:
    if cash - bet >= 0 :
        print()
        selection = int(input("Would you like to split your cards? "))
    else:
